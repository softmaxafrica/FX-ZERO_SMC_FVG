...
// ======================================================================
string Inp_TFs             = "H1";      // default scan set (kept for reference)
input int    Inp_LookbackBars    = 1000;
input int    Inp_ScanOnTimerSec  = 0;        //  (new bar only)
input bool   Inp_MergeOverlaps   = false;
input bool   Inp_ExtendToRight   = true;
input int    Inp_MaxZonesPerTF   = 200;

// Quality/Spread gates for FVGs
input int    Inp_MinPoints       = 30;       // slightly looser for more valid gaps
input bool   Inp_UseATRFilter    = true;
input bool   Inp_UseATRforSpread = true;
input double Inp_MaxSpreadATRMult= 1;     // allow moderate spreads on indices (ATR * 0.75)
...
input double   Inp_DisplacementMin = 2;
input double   Inp_MaxDistATR      = 1.5;                  // balanced distance to mid
input double   Inp_ScoreMin        = 0.58;                 // balanced composite score
...
// Legacy + SL padding
const double   Inp_RiskPerTradePct = 0;         // 0 to disable the hard cap (legacy)
...
input EEntryMode  Inp_EntryMode  = Entry_Market;
...
input EExitMode Inp_ExitMode       = Exit_BE_Trail;  // trailing with delayed BE

// BE/Trail parameters (used only if Exit_BE_Trail)
input double Inp_BE_Trig_R    = 1.0;   // BE later to avoid scratches on indices
input double Inp_Trail_Trig_R = 2.0;   // start trailing a bit later
const int    Inp_Trail_SwingK = 3.0;


// ======================================================================
// 9) CUSTOM CONFIRMATION ENGINE — K-of-N or Pairs
// ======================================================================
input bool Inp_UI_UseStructured = true;

// K-of-N toggles (enable what you want), then choose K
input bool Inp_UI_Confirm_ENGULF = true;
input bool Inp_UI_Confirm_MFVG   = true;
input bool Inp_UI_Confirm_WICK   = true;
input bool Inp_UI_Confirm_BOS    = true;
input bool Inp_UI_Confirm_SFP    = true;
input bool Inp_UI_Confirm_RETEST = true;
input bool Inp_UI_Confirm_IB     = true;
input int  Inp_UI_Confirm_K      = 2;      // Allow best single confirmation to trigger


enum EConfirmation { CONF_ENGULF=0, CONF_MFVG=1, CONF_WICK=2, CONF_BOS=3, CONF_SFP=4, CONF_RETEST=5, CONF_IB=6, CONF_NONE=7 };
input bool          Inp_UI_UsePairs          = true;  // AB Pairs instead of K-of-N
input bool          Inp_ConfirmPair1_Enable  = true;
input EConfirmation Inp_ConfirmPair1_A       = CONF_BOS;
input EConfirmation Inp_ConfirmPair1_B       = CONF_MFVG;
input bool          Inp_ConfirmPair2_Enable  = true;
input EConfirmation Inp_ConfirmPair2_A       = CONF_RETEST;
input EConfirmation Inp_ConfirmPair2_B       = CONF_ENGULF;
input bool          Inp_ConfirmPair3_Enable  = true;
input EConfirmation Inp_ConfirmPair3_A       = CONF_BOS;
input EConfirmation Inp_ConfirmPair3_B       = CONF_SFP;
input bool          Inp_ConfirmPair4_Enable  = true;
input EConfirmation Inp_ConfirmPair4_A       = CONF_BOS;
input EConfirmation Inp_ConfirmPair4_B       = CONF_RETEST;
input bool          Inp_ConfirmPair5_Enable  = true;
input EConfirmation Inp_ConfirmPair5_A       = CONF_BOS;
input EConfirmation Inp_ConfirmPair5_B       = CONF_IB;
input bool          Inp_ConfirmPair6_Enable  = true;
input EConfirmation Inp_ConfirmPair6_A       = CONF_ENGULF;
input EConfirmation Inp_ConfirmPair6_B       = CONF_SFP;
...
input ENUM_TIMEFRAMES Inp_BOS_Child_M15 = PERIOD_M5;
...
input int      Inp_CleanFVG_AfterHours = 48;
...
// Trim detail to keep line readable
  ...
  PrintFormat("[ZERO_TRADE]|PH=%s|TIME=%s|SYM=%s|TF=%s|SIDE=%s|MODE=%s|TICKET=%I64u|LOTS=%.3f|ENTRY=%.*f|SL=%.*f|TP=%.*f|CONF=%s|FT:%c|ZTF=%s|ZTOP=%.*f|ZBOT=%.*f|ZTIME=%s|%s|DETAIL=%s",
               ...
               TFToStr(s.htf),
               SideStr(s.is_buy),
               (Inp_EntryMode==Entry_PendingAtEntry ? "PEND" : "MKT"),
               ticket,
               lots,
               DigitsSym(), entry_ref,
               DigitsSym(), sl,
...
// 1) Get server time as a struct (works live & in tester)
   MqlDateTime dt;
   TimeToStruct(TimeCurrent(), dt);     // or TimeTradeServer() if you prefer

// 2) Apply your broker/session offset (hours), then normalize to 0..23
   int h = dt.hour + Inp_Session_TZ_Offset_Hours;
   while(h < 0)
      h += 24;
   while(h >= 24)
      h -= 24;

// 3) Session windows (support ranges that cross midnight)
   bool inLondon = Inp_Session_Allow_London &&
                   ((Inp_London_Start_Hour <= Inp_London_End_Hour)
                    ? (h >= Inp_London_Start_Hour && h < Inp_London_End_Hour)
                    : (h >= Inp_London_Start_Hour || h < Inp_London_End_Hour));

   bool inNY1    = Inp_Session_Allow_NY_FirstHalf &&
                   ((Inp_NY1_Start_Hour <= Inp_NY1_End_Hour)
                    ? (h >= Inp_NY1_Start_Hour && h < Inp_NY1_End_Hour)
                    : (h >= Inp_NY1_Start_Hour || h < Inp_NY1_End_Hour));

   return (inLondon || inNY1);
  }
...
int ConfirmTokenIndex(const string &tok_in)
  {
   string tok = Upper(Trim(tok_in));  // OK now (both take by value)
   if(tok=="ENGULF")
      return 0;
   if(tok=="MFVG")
      return 1;
...
// priority (first matching wins). You can reorder if you like.
   string prio[] = {"MFVG","ENGULF","BOS","SFP","RETEST","IB"};
...
// displacement using the impulse bar that closed the gap
bool HTF_DisplacementFactor(ENUM_TIMEFRAMES tf, datetime t1, double &disp)
  {
   disp=0.0;
   int bars;
   MqlRates r[];
   if(!CopyRatesTF(tf, 80, r, bars))
      return false;
...
bool LTF_Engulf(bool is_buy)
  {
   MqlRates r[];
   int bars;
   if(!CopyRatesTF(TF_LTF(), 5, r, bars) || bars<3)
      return false;

   double c1c=r[1].close, c1o=r[1].open, pHi=r[2].high, pLo=r[2].low;
   return is_buy ? (c1c>pHi && c1c>c1o) : (c1c<pLo && c1c<c1o);
  }
...
// ==== BOS on child TF within the LTF bar window (heuristic & robust) ====
// bool LTF_BOSAt(bool is_buy, int s)
//   {
// // LTF window
//    datetime t_open = iTime(_Symbol, TF_LTF(), s);
//    datetime t_close= iTime(_Symbol, TF_LTF(), s-1);
...
//    int idxStart = iBarShift(_Symbol, ctf, t_open,  true);
//    int idxEnd   = iBarShift(_Symbol, ctf, t_close, true);
//    if(idxStart<0 || idxEnd<0)
//       return false;

// // prior reference: last ~30 child bars *before* the window start
//    double priorHi = -DBL_MAX, priorLo = DBL_MAX;
//    int lookback=30;
//    for(int i=idxStart+1; i<=idxStart+lookback && i<cb; ++i)
//      {
//       if(cr[i].high>priorHi)
//          priorHi=cr[i].high;
//       if(cr[i].low <priorLo)
//          priorLo=cr[i].low;
//      }
//    if(priorHi==-DBL_MAX || priorLo==DBL_MAX)
//       return false;

// // inside the window: look for a close beyond prior ref (struct break)
//    int i0 = MathMax(idxEnd, 1);
//    for(int i=i0; i<=idxStart && i<cb; ++i)
//      {
//       if(is_buy)
//         {
//          if(cr[i].close > priorHi)
//             return true;
//         }
//       else
//         {
//          if(cr[i].close < priorLo)
//             return true;
//         }
//      }
//    return false;
...
bool LTF_RetestThresholdAt(bool is_buy, int s, double top, double bot, ENUM_TIMEFRAMES htf)
  {
   ...
   double th = FvgThreshold(top, bot, is_buy);
   double hi=r[s].high, lo=r[s].low, op=r[s].open, cl=r[s].close;
   bool touched = (lo <= th && hi >= th);
   if(!touched)
      return false;

// close away from threshold, in the intended direction
   if(is_buy)
      return (cl > th && cl >= op);
   else
      return (cl < th && cl <= op);
...
// ---- diagnostics (compact but rich) ----
   ...
   string mode                 = (Inp_EntryMode==Entry_PendingAtEntry? "PEND" : "MKT");
   ...
   string advanced = StringFormat(
                        ...
                        confirmTag, flagsStr, mode, (StringLen(anchorNm)>0 ? anchorNm : "—"),
...
bool LTF_EngulfAt(bool is_buy, int s)
  {
   ...
   if(!CopyRatesTF(TF_LTF(), s+3, r, bars) || bars < s+2)
      return false;
   double c1c = r[s].close, c1o = r[s].open;
   double pHi = r[s+1].high,  pLo = r[s+1].low;
   return is_buy ? (c1c>pHi && c1c>c1o) : (c1c<pLo && c1c<c1o);
  }
//+------------------------------------------------------------------+
...
// displacement & sweep based on bar that closed the gap
   ...
   if(disp < Inp_DisplacementMin)
      return false;
   bool swept = HTF_SweepHeuristic(tf, t1, is_buy);
...
void DrawSessionLines_Today_Simple()
  {
// 0) remove just our 6 known lines so only TODAY remains
   const string NAMES[6] =
     {
      "ZERO_SES_ASIA_S","ZERO_SES_ASIA_E",
      "ZERO_SES_LON_S","ZERO_SES_LON_E",
      "ZERO_SES_NY1_S","ZERO_SES_NY1_E"
     };
   for(int i=0;i<6;i++)
      if(ObjectFind(0,NAMES[i])>=0)
         ObjectDelete(0,NAMES[i]);

// 1) today's midnight in SERVER time (chart axis = server time)
   datetime nowSrv = TimeCurrent();
   MqlDateTime ds;
   TimeToStruct(nowSrv, ds);
   ds.hour=0;
   ds.min=0;
   ds.sec=0;
   datetime dayStartSrv = StructToTime(ds);

// 2) server offset vs UTC (rounded to nearest hour)
   int serverUTC_H = (int)MathRound((double)(TimeCurrent() - TimeGMT()) / 3600.0);

// 3) convenience
   const int offH = Inp_Session_TZ_Offset_Hours; // your local (e.g., TZ +3 for Tanzania)
   const long styleL = (long)Inp_SessionLineStyle;
   const long widthL = (long)Inp_SessionLineWidth;

// ---- a tiny inline “normalize to 0..23” (no helper function) ----
// (repeat where needed, to keep it one method)
// ---------------------------------------------------------------

// ====== ASIA ======
   if(Inp_Session_Allow_Asia)
     {
      int hS = Inp_Asia_Start_Hour;
      int hE = Inp_Asia_End_Hour;
      if(Inp_Session_Hours_Are_UTC)
        {
         hS = hS + offH - serverUTC_H;
         hE = hE + offH - serverUTC_H;
        }
      else
        {
         hS = hS - serverUTC_H;
      ...
      datetime tS = dayStartSrv + (hS*3600);
      datetime tE = dayStartSrv + (hE*3600);

      string n1="ZERO_SES_ASIA_S", n2="ZERO_SES_ASIA_E";
      ObjectCreate(0,n1,OBJ_VLINE,0,tS,0.0);
      ObjectSetInteger(0,n1,OBJPROP_COLOR,(long)Inp_SessionColor_Asia);
      ObjectSetInteger(0,n1,OBJPROP_WIDTH,widthL);
      ObjectSetInteger(0,n1,OBJPROP_STYLE,styleL);
      ObjectSetInteger(0,n1,OBJPROP_BACK,true);
      ObjectSetInteger(0,n1,OBJPROP_SELECTABLE,false);

      ObjectCreate(0,n2,OBJ_VLINE,0,tE,0.0);
      ObjectSetInteger(0,n2,OBJPROP_COLOR,(long)Inp_SessionColor_Asia);
      ObjectSetInteger(0,n2,OBJPROP_WIDTH,widthL);
      ObjectSetInteger(0,n2,OBJPROP_STYLE,styleL);
...
// ====== LONDON ======
   if(Inp_Session_Allow_London)
     {
      int hS = Inp_London_Start_Hour;
      int hE = Inp_London_End_Hour;
      if(Inp_Session_Hours_Are_UTC)
        {
         hS = hS + offH - serverUTC_H;
         hE = hE + offH - serverUTC_H;
      ...
      string n1="ZERO_SES_LON_S", n2="ZERO_SES_LON_E";
      ObjectCreate(0,n1,OBJ_VLINE,0,tS,0.0);
      ObjectSetInteger(0,n1,OBJPROP_COLOR,(long)Inp_SessionColor_London);
      ObjectSetInteger(0,n1,OBJPROP_WIDTH,widthL);
      ObjectSetInteger(0,n1,OBJPROP_STYLE,styleL);
      ObjectSetInteger(0,n1,OBJPROP_BACK,true);
      ObjectSetInteger(0,n1,OBJPROP_SELECTABLE,false);
      ...
      ObjectCreate(0,n2,OBJ_VLINE,0,tE,0.0);
      ObjectSetInteger(0,n2,OBJPROP_COLOR,(long)Inp_SessionColor_London);
      ObjectSetInteger(0,n2,OBJPROP_WIDTH,widthL);
      ObjectSetInteger(0,n2,OBJPROP_STYLE,styleL);
      ObjectSetInteger(0,n2,OBJPROP_BACK,true);
      ObjectSetInteger(0,n2,OBJPROP_SELECTABLE,false);
...
// ====== NEW YORK (first half) ======
   if(Inp_Session_Allow_NY_FirstHalf)
     {
      int hS = Inp_NY1_Start_Hour;
      int hE = Inp_NY1_End_Hour;
      if(Inp_Session_Hours_Are_UTC)
        {
         hS = hS + offH - serverUTC_H;
         hE = hE + offH - serverUTC_H;
      ...
      string n1="ZERO_SES_NY1_S", n2="ZERO_SES_NY1_E";
      ObjectCreate(0,n1,OBJ_VLINE,0,tS,0.0);
      ObjectSetInteger(0,n1,OBJPROP_COLOR,(long)Inp_SessionColor_NY1);
      ObjectSetInteger(0,n1,OBJPROP_WIDTH,widthL);
      ObjectSetInteger(0,n1,OBJPROP_STYLE,styleL);
      ObjectSetInteger(0,n1,OBJPROP_BACK,true);
      ObjectSetInteger(0,n1,OBJPROP_SELECTABLE,false);
      ...
      ObjectCreate(0,n2,OBJ_VLINE,0,tE,0.0);
      ObjectSetInteger(0,n2,OBJPROP_COLOR,(long)Inp_SessionColor_NY1);
      ObjectSetInteger(0,n2,OBJPROP_WIDTH,widthL);
      ObjectSetInteger(0,n2,OBJPROP_STYLE,styleL);
      ObjectSetInteger(0,n2,OBJPROP_BACK,true);
      ObjectSetInteger(0,n2,OBJPROP_SELECTABLE,false);
...
// double ToTick(double p){ return NormalizeDouble(p,DigitsSym()); }


// Enforce broker stop level; keep tp==0 as “no TP”
// void FixStopsToLevels(double entry, bool is_buy, double &sl, double &tp)
//   {
//    const int    stopsLvlPts = (int)SymbolInfoInteger(_Symbol, SYMBOL_TRADE_STOPS_LEVEL);
//    const double minDist     = stopsLvlPts * _Point;

//    if(is_buy)
//      {
//       if(entry - sl < minDist)
//          sl = entry - (minDist + 2*_Point);
//       if(tp > 0.0 && (tp - entry < minDist))
//          tp = entry + (minDist + 2*_Point);
//      }
//    else
//      {
//       if(sl - entry < minDist)
